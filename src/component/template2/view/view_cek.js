...
window.value_cek = (data_pembayaran) => { 
    try {
        ...
        const total_bayar = document.querySelector('#total_bayar')
        ...
        data_pembayaran.rincian_pembelian.map(item => {
            ...
            <div class="col-8 mb-1 text-end">
                <p class="font-large text-secondary">${formatRupiah(item.value_admin,'Rp. ')}</p>
            </div>
            <div class="col-4 mb-1">
                <p class="font-large text-secondary">${item.title_tagihan}</p>
            </div>
            <div class="col-8 mb-1 text-end">
                <p class="font-large text-secondary">${formatRupiah(item.value_tagihan,'Rp. ')}</p>
            </div>
            <div class="col-4 mb-1">
                <p class="font-large text-secondary">${item.title_total_tagihan}</p>
            </div>
            <div class="col-8 mb-1 text-end">
                <p class="font-large text-secondary">${ formatRupiah(item.value_total_tagihan,'Rp. ') }</p>
            </div>
            <div class="col-4 mb-1">
        ...
        view_rincian_bayar = `
            ...
            <div class="col-8 mb-1 text-end">
            <p id="saldo-akhir" class="font-large text-dark">${(typeof data_pembayaran.detail_pembayaran.value_saldo_akhir === "string") ? data_pembayaran.detail_pembayaran.value_saldo_akhir : formatRupiah(data_pembayaran.detail_pembayaran.value_saldo_akhir,'Rp. ')}</p>
            </div>
        `
