...
const setupview = (data,produks) => {
    ...
    if(is_scan){  
        ...
        if(is_kontak === true){
          scan_mode = `<span id="labelContact" onclick="kontak(this)" class="input-group-text bg-transparent border-start-0 py-1 px-2">
    ...
    global.dynamic_form_1 = `<div class="col-12 mt-2 dynamic-form">
        <div class="multi">
            <div class="row">
                ...
                <input type="text" onkeypress="return isNumber(event,this);" class="form-control border-end-0 shadow-none border-radius-left-8 font-large-1 text-dark nomor" placeholder="${hint}" aria-label="Nomor User" inputmode="${keyboard_mode}"  aria-describedby="inptNomor" autocomplete="off" />
                ${scan_mode}
                </div>
                ...
                <div class="col-12 pt-1">
                  <div class="input-group mb-1">
                    <input type="text" class="form-control border-end-0 bg-white shadow-none border-radius-left-8 font-large-1 text-dark produk" aria-label="Nominal" aria-describedby="inptproduk" autocomplete="off" value="${produk}" readonly data-produk="${produk}" data-harga="${fee}" data-kode="${kode}" data-exp="${exp}"/>
    ...
    footer_button = `<div class="col-12 px-1">
    ${cek_button}
    </div>
    `
    total_bottom.innerHTML = `(${rupiah.formatRupiah(Math.abs(fee),'Rp. ')})` 
